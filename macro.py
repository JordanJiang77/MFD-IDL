...
def get_speed_mean_field(input_filename, dx, dt, starttime, endtime, lane_number, min_milemarker = 58.7, testbed_mile = 4):
    ...
    density_raw = pd.DataFrame(TTT_matrix)
    flow_raw = pd.DataFrame(TTD_matrix)
    speed_raw = pd.DataFrame(matrix_to_coordinates(3600 * TTD_matrix / TTT_matrix))
    # density_raw.columns = ['t', 'x', 'density']
    # flow_raw.columns = ['t', 'x', 'flow']
    speed_raw.columns = ['t', 'x', 'speed']
    return density_raw, flow_raw, speed_raw
...
dx = 0.1
dt = 10
starttime = 1669123197
endtime = 1669123237
# lane_number = 2
density1, flow1, speed1 = get_speed_mean_field(input_filename, dx, dt, starttime, endtime, lane_number=1, min_milemarker = 58.7, testbed_mile = 4)
density2, flow2, speed2 = get_speed_mean_field(input_filename, dx, dt, starttime, endtime, lane_number=2, min_milemarker = 58.7, testbed_mile = 4)
density3, flow3, speed3 = get_speed_mean_field(input_filename, dx, dt, starttime, endtime, lane_number=3, min_milemarker = 58.7, testbed_mile = 4)
density4, flow4, speed4 = get_speed_mean_field(input_filename, dx, dt, starttime, endtime, lane_number=4, min_milemarker = 58.7, testbed_mile = 4)
print(speed2)

speed1 = np.array(speed1['speed'])
speed2 = np.array(speed2['speed'])
speed3 = np.array(speed3['speed'])
speed4 = np.array(speed4['speed'])
n_segments = len(speed2) // 40
speed1d = speed1[:n_segments*40].reshape(-1, 40)
speed2d = speed2[:n_segments*40].reshape(-1, 40)
speed3d = speed3[:n_segments*40].reshape(-1, 40)
speed4d = speed4[:n_segments*40].reshape(-1, 40)
speed = np.concatenate((speed1d, speed2d, speed3d, speed4d), axis=1)
print(speed)
print(speed.shape)

# speed_1 = np.concatenate((speed1, speed2), axis=1)
# speed_2 = np.concatenate((speed3, speed4), axis=1)
# speed = np.concatenate((speed_1, speed_2), axis=1)
# flow_1 = np.concatenate((flow1, flow2), axis=1)
# flow_2 = np.concatenate((flow3, flow4), axis=1)
# flow = np.concatenate((flow_1, flow_2), axis=1)
# density_1 = np.concatenate((density1, density2), axis=1)
# density_2 = np.concatenate((density3, density4), axis=1)
# density = np.concatenate((density_1, density_2), axis=1)
# print(flow.shape)

# nodes = flow.shape[1]
# lane_nodes = nodes // 4
# adj = np.zeros((nodes, nodes))
# for i in range(lane_nodes):
#     adj[i][i-1]=adj[i][i+1]=1
#     adj[i][i+lane_nodes]=1
#     # adj[i][i+lane_nodes-1] = adj[i][i+lane_nodes+1]=0.5
# for i in range(nodes-lane_nodes, nodes-1):
#     adj[i][i-1]=adj[i][i+1]=1
#     adj[i][i-lane_nodes]=1
#     # adj[i][i-lane_nodes+1] = adj[i][i-lane_nodes-1]=0.5
# for i in range(lane_nodes,nodes-lane_nodes):
#     adj[i][i-1]=adj[i][i+1]=1
#     adj[i][i-lane_nodes]=adj[i][i+lane_nodes]=1
#     # adj[i][i-lane_nodes-1]=0.5
#     # adj[i][i-lane_nodes+1]=0.5
#     # adj[i][i+lane_nodes-1]=0.5
# print(adj)
...
speed.to_excel('speed10.xlsx', index=False, header=True)

# flow.to_excel('flowdemo.xlsx', index=False, header=True)
#
# density.to_excel('densitydemo.xlsx', index=False, header=True)
# adj.to_excel('adjdemo.xlsx', index=False, header=True)